...
void FRAMES::load_circle(Mat cur_frame, Mat cur_depth, int n0) {
	...
	string csv_path_s = temp_data_path + to_string(n0) + ".csv";
	Public_func pf;
	if(pf.exists_file(csv_path_s)){
		ifstream csvp(csv_path_s.c_str());
		string s;
		char c;
		getline(csvp, s, ',');
		int r = 0, x = 0, y = 0;
